...
Then('the patient of the created order should have an identification with code {string} assigned by {string}') do |string, string2|
  ...
  # Verify the glims db
  ins_nir = Correspondent.first(crsp_internalId: string2)
  expect(ins_nir).not_to be_nil
  ...
  expect(patient.correspondent.assigned_to_identifications(idnt_code: string, idnt_source: ins_nir.crsp_id).length).to eq(1)
end
